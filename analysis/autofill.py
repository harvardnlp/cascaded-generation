...
latencies = [318.85, 294.64, 229.76]
distilled = True
valid = False
...
with open(sys.argv[1]) as fin:
    with open(sys.argv[2], 'w') as fout:
        for line in fin:
            ...
            output_items = []
            output_items.append(f'Barrier + Cascade (K={topk}, rounds={rounds})')
            ...
            for i in range(3):
                ...
                if os.path.exists(log_file_1) and os.path.exists(log_file_2):
                    ...
                    speedup = latencies[i] / latency 
                    speedup = round(speedup, 2)

                    #print (bleu, score, latency)
                    #output_items.append(f' {bleu} ({score})\t')
                    output_items.append(f' {bleu}\t')
                    output_items.append(f' {latency}ms ({speedup})\t')
                else:
                    print (log_file_1)
                    output_items.append(' \t')
                    output_items.append(' \t')
